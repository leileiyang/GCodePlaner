...
void RS_Grid::updatePointArray() {
    ...
    if (true /*graphic*/) {
        ...
        isometric = false; //static_cast<bool>(graphic->getVariableInt("$SNAPSTYLE",0));
        crosshairType = RS2::LeftCrosshair; //graphic->getCrosshairType();
        userGrid = RS_Vector(-1.0, -1.0); //graphic->getVariableVector("$GRIDUNIT", RS_Vector(-1.0, -1.0));
	}else {
    ...
    if (true /*graphic*/) {
        unit = RS2::Millimeter;//graphic->getUnit();
        format = RS2::Decimal;//graphic->getLinearFormat();
	}
